...
class TypeREAL extends VarType implements TypeInterface {
	...
	public readonly SUPPORTED_ST_VALUES = [-3.402823466E+38, +3.402823466E+38];
	constructor(
		column: number,
		line: number,
		value: string | null,
	) {
		super(column, line, 'REAL', value);
	}

	static create(column: number, line: number, value: string | null): TypeInterface {
		const preparedValue = value?.replace('_', '') ?? null;
		return new TypeREAL(column, line, preparedValue);
	}
